...
def k3(log = False):
    ...
    while run:
        #print('mda')
        win.fill((0,0,0))
        win.blit(pygame.font.SysFont('Comic Sans MS', 22).render('Введите name(6 max)', False, (250, 250, 250)), (10, 140))
        for bb in blits:
            win.blit(bb, (12*zz, 160))
            zz += 1
        zz = 1
...
def reg_pas(log = False):
    run = True
    blits = []
    while run:
        win.fill((0,0,0))
        win.blit(pygame.font.SysFont('Comic Sans MS', 22).render('Введите pass(6 max)', False, (250, 250, 250)), (10, 140))
        for bb in blits:
            win.blit(bb, (12*zz, 160))
            zz += 1
        zz = 1
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    while loop:
                        try:
                            sock = socket.create_connection((ip, 9191))
                            loop = False
                        except ConnectionRefusedError:
                            if i == 0:
                                print('-------------------------------------------------------------------')
                            ...
                            print(eq)
                        if log:
                            sock.send(f'log:{name}:{pas}:0:0'.encode())
                        else:
...
while run:
    try:
        win.fill((0,0,0))
        win.blit(pygame.font.SysFont('Comic Sans MS', 30).render('1:Log', False, (250, 250, 250)), (10, 120))
        win.blit(pygame.font.SysFont('Comic Sans MS', 30).render('2:Reg', False, (250, 250, 250)), (10, 150))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
